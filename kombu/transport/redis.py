...
class Channel(virtual.Channel):
    ...
    def _connparams(self, async_=False, _r210_options=(
    ...
    def _get_pool(self, async_=False):
        params = self._connparams(async_=async_)
    ...
    def async_pool(self):
        if self._async_pool is None:
            self._async_pool = self._get_pool(async_=True)
        return self._async_pool

    @cached_property
    def client(self):
        """Client used to publish messages, BRPOP etc."""
        return self._create_client(async_=True)

    @cached_property
    def subclient(self):
        """Pub/Sub connection used to consume fanout queues."""
        client = self._create_client(async_=True)
